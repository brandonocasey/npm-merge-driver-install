...
test("not installed with bad git binary (before install)", (t) => {
...
if (os.platform() !== "win32") {
  test("exit code 0 if installed and run as binary", (t) => {
    ...
    return promiseSpawn(path.join(BASE_DIR, "src", "is-installed.js"), [], { cwd: t.context.dir }).then((result) => {
  ...
  test("exit code 1 if not installed and run as binary", (t) => {
    return promiseSpawn(path.join(BASE_DIR, "src", "is-installed.js"), [], {
      cwd: t.context.dir,
      ignoreExitCode: true,
    }).then((result) => {
...
test("exit code 0 if installed and run with node", (t) => {
  ...
  return promiseSpawn("node", [path.join(BASE_DIR, "src", "is-installed.js")], { cwd: t.context.dir }).then(
    (result) => {
      t.is(result.exitCode, 0);
      t.is(result.stderr, "");
      t.is(result.stdout, "");
...
test("exit code 1 if not installed and run with node", (t) => {
  return promiseSpawn("node", [path.join(BASE_DIR, "src", "is-installed.js")], {
    cwd: t.context.dir,
    ignoreExitCode: true,
  }).then((result) => {
    t.is(result.exitCode, 1);
