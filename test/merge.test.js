...
test.beforeEach((t) => {
  ...
  return t.context.install().then(function() {
    ...
    console.log(shell.cat(path.join(t.context.dir, '.git', 'info', 'attributes')).stdout);
    // eslint-disable-next-line
    console.log(shell.cat(path.join(t.context.dir, '.git', 'config')).stdout);
    return promiseSpawn('git', ['add', '--all'], {cwd: t.context.dir});
  }).then(function() {
    return promiseSpawn('git', ['commit', '-a', '-m', '"install npm-merge-driver-install"'], {cwd: t.context.dir});
  });
});
